...
<div class="home-container">
  <div class="chats">
    <div class="chat" v-for="chat of chats" :class="chat.ROWID==conversations_current_chatId ? 'selected' : ''">
      .chat-avatar
        i.material-icons account_circle
      .chat-name
        p: a(href="#", @click="loadMessage(chat.chat_identifier, chat.ROWID)") {{ chat.dName }}
      .chat-time
    </div>
  </div>
  .conversations
    .message-container
      p(v-if="messagesLoading") Loading
      p(v-else-if="conversations_current.length == 0") No Messages Found
      .message-wrapper(
        v-else,
  ...
  methods: {
    load() {
      ...
      if (this.conversations_current_target) {
        this.loadMessage(
          this.conversations_current_target,
          this.conversations_current_chatId
        );
      }
    },
